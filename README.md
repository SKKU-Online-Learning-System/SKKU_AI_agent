...
성균관대학교 AI중심대학사업의 강의자료 기반 RAG 코스 에이전트 MVP입니다. 학생은 과목을 선택해 출처 포함 답변을 받고, 교수자는 자료 처리와 담당 과목 로그·통계를 확인하며, 관리자는 과목·사용자·전체 로그·서비스 통계를 관리합니다.
...
- 임베딩 생성: API 키 없이 동작하는 결정적 local hash 제공자
...
- 관리자 대시보드: 전체 과목·사용자·질문, 일자별 질문, 과목별 질문·사용자·자료 수
- 교수자 대시보드: 담당 과목별 자료·질문 수, 최근 질문, 단순 빈도 기반 상위 키워드
- 학생 본인 질문·대화·과목별 사용 통계 API
- 관리자 전체 로그, 교수자 담당 과목 로그, 학생 본인 로그의 역할별 범위 강제
- 로그 검색어·기간·과목·사용자·자료 근거 여부 필터
- 역할별 메뉴, 로딩·오류·빈 상태, 답변 출처·안전 경고 UI
...
- AI/RAG: local hash 임베딩·과목 범위 검색·Claude 답변 생성, mock LLM 지원
...
루트 `.env`는 API/DB 설정 파일입니다. 기본값은 `APP_ENV=local`, 로컬 Docker DB, 프런트엔드 Origin(`http://localhost:3000`)을 대상으로 합니다. 예시 `JWT_SECRET`은 로컬 개발 전용입니다. `APP_ENV`가 `local`이 아닌 환경에서는 예시 키를 사용할 수 없으며, 고유하게 생성한 32자 이상의 임의 키로 교체해야 API가 시작됩니다. 실제 배포 전에는 `ANTHROPIC_API_KEY`도 설정하고, `.env`는 커밋하지 마세요.
...
| `SEED_PASSWORD` | `password123` | 로컬 Seed 계정 공통 비밀번호. 공유 환경에서는 반드시 변경 |
...
| `MOCK_EMBEDDING_DIM` | `512` | local hash 임베딩 차원 |
...
| `CLAUDE_MODEL` | `claude-sonnet-5` | 실제 답변 생성 모델명 |
| `USE_MOCK_LLM` | `true` | `true`면 API 키 없이 결정적 mock 답변 사용 |
| `LLM_TEMPERATURE` / `LLM_MAX_TOKENS` | `0.2` / `1024` | 답변 생성 파라미터 |
| `MAX_QUESTION_LENGTH` | `2000` | 질문 최대 길이 |

`USE_MOCK_LLM=false`로 두고 `ANTHROPIC_API_KEY`가 없으면 답변 생성은 명확한 오류를 반환합니다.
...
Seed는 `인공지능개론`, `소프트웨어공학`과 학생 수강 권한을 멱등 생성합니다. 업로드·처리 흐름 확인용 텍스트는 `docs/samples/ai-intro-sample.txt`입니다.
...
| GET              | `/api/student/chat-logs`                                             | 학생, 200                | 본인 질문 로그(기간·과목 필터)             |
| GET              | `/api/stats/service`                                                  | 관리자, 200              | 전체 서비스와 과목별 통계                  |
| GET              | `/api/stats/courses/{course_id}`                                      | 관리자·담당 교수자, 200  | 단일 과목 통계                             |
| GET              | `/api/stats/professor`                                                | 교수자, 200              | 담당 과목·최근 질문·키워드 통계            |
| GET              | `/api/stats/me`                                                       | 인증 사용자, 200         | 본인 사용 통계                             |
...
기본 환경(`USE_MOCK_LLM=true`)에서는 Claude API 키 없이 전체 흐름을 확인할 수 있습니다. 업로드용 예시 자료는 `docs/samples/ai-intro-sample.txt`에 있으며, 실제 업로드 흐름을 그대로 사용합니다.
...
- [ ] 교수자 대시보드에서 담당 과목별 자료·질문 수, 최근 질문, 키워드가 보인다.
- [ ] 관리자 대시보드에서 전체 수치, 일자별 질문, 과목별 사용량이 보인다.

## 아직 구현되지 않은 기능

다음은 현재 범위 밖이며, 화면 문구나 빈 응답을 실제 기능으로 해석하면 안 됩니다.

- 로그 CSV 내보내기와 통계 차트 시각화
- 형태소 분석 기반 한국어 키워드 정규화 (현재 최근 1,000개 질문의 단순 토큰 빈도)
