...
* answers are generated by this project's ``LLMService`` (Qwen, or the
  ...
  Only the realtime speech-to-speech leg still runs on xAI Grok.
...
def text_agent_tools() -> list[dict]:
    """Return action tools in the OpenAI-compatible shape used by Qwen and Grok."""
    return [
        tool
        for tool in TOOLS
...
async def search_trusted_web(
    ...
    """Search the course allowlist through Qwen and retain citation audit data.
...
def _conversation(history: list[dict]) -> list[dict]:
    """Return the history trimmed to a valid Qwen conversation.
...
async def think(
    ...
    """Generate one grounded Socratic response through Qwen's tool use.
